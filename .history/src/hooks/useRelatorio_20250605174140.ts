...
import type { LinhaRelatorio, SerieInfo } from "../types";
...
export function useRelatorio() {
  ...
  // 3) Ao montar, lê storage e converte em LinhaRelatorio[]
  useEffect(() => {
  ...
  const excluirLinha = (idx: number) => {
    setLinhas((prev) => {
      ...
      const dados = carregarDados();
      const cid = CICLOS.find((c: { id: string; titulo: string }) => c.titulo === lin.ciclo)?.id;
