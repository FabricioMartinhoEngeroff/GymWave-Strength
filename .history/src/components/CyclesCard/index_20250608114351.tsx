...
export default function CycleCard({ value, onSave }: CycleCardProps) {
  ...
  {exercicioSelecionado && sugestaoPeso > 0 ? (
    <>
      💪 Com base no seu histórico e no ciclo selecionado, sugerimos iniciar com aproximadamente{" "}
      <span style={{ fontWeight: "bold", fontSize: "1.2em" }}>{sugestaoPeso} kg</span> neste exercício.
      <br />
      🎯 <em>Objetivo do ciclo:</em> {cicloInfo.objetivo}
    </>
  ) : (
    <>
      Selecione um exercício para ver a sugestão de carga com base no seu histórico.
    </>
  )}
</MensagemMotivacional>
...
function calcularPesoSugestivo(ciclo: CicloInfo, exercicio: string): number {
  ...
  const pesos = ciclo4.pesos.map((p: string) => parseFloat(p)).filter((n: number) => !isNaN(n));
  const pico = pesos.length ? pesos.reduce((a: number, b: number) => a + b, 0) / pesos.length : 0;

  if (!pico) return 0;

  const percentual = ciclo.percentual;

  if (percentual.includes("20%")) return Math.round(pico * 0.83);
  if (percentual.includes("-10%")) return Math.round(pico * 0.9);
  if (percentual.includes("0%")) return Math.round(pico);
  if (percentual.includes("+5%")) return Math.round(pico * 1.05);

  return Math.round(pico);
}
