...
/**
 * Parâmetros que o tick do eixo X recebe.
 */
type TickProps = {
  x?: number;
  y?: number;
...
/**
 * Conteúdo customizado do Tooltip: exibe data, linhas de séries, total e média.
 */
 ...
 * Se não houver payload ou payload.value, retorna um <g /> vazio.
 */
const RenderizarTickX: React.FC<any> = ({ x = 0, y = 0, payload }) => {
  if (!payload?.value) {
    return <g />;
  }
  ...
  const cycle = cycleWithParens ? cycleWithParens.replace(/[()]/g, "") : "";
  const yOffset = 4;
