...
// Tooltip personalizado (igual ao seu)
...
export default function Graphics() {
  ...
  useEffect(() => {
    ...
    Object.entries(bruto).forEach(([exe, ciclos]: any) => {
      Object.entries(ciclos).forEach(([cicloKey, reg]: any) => {
        const pesosNum = (reg.pesos||[]).map((p:string)=>parseFloat(p)||0);
        const repsNum  = (reg.reps||[]).map((r:string)=>parseInt(r)||0);
