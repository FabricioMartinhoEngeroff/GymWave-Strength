...
export default function Graphics() {
  ...
  Object.entries(bruto).forEach(([exe, ciclos]) => {
    Object.entries(ciclos).forEach(([cicloKey, reg]) => {
      ...
      // busca as infos do ciclo e monta o título
      const cicloInfo   = CICLOS.find(c => c.id === cicloKey);
      const cicloTitulo = cicloInfo?.titulo ?? cicloKey;

      // monta a label incluindo a data e o título do ciclo
      const labelData = `${reg.data.slice(0,5)} (${cicloTitulo})`;
  ...
  // ordena cronologicamente pelo dia/mês
  Object.values(agrup).forEach(arr =>
    arr.sort((a, b) => {
      ...
      return new Date(2025, ma - 1, da).getTime()
           - new Date(2025, mb - 1, db).getTime();
    })
  );
