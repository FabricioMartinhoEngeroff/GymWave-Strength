      ...
      <div style={{maxWidth:500,margin:"0 auto"}}>
        {linhasFiltradas.map((l,idx)=>(
          <div key={idx} style={{background:"#fff",padding:16,borderRadius:12,boxShadow:"0 2px 6px rgba(0,0,0,0.08)",marginBottom:20,border:"1px solid #e2e8f0"}}>
            {editandoIdx===idx ? (
              <>
                {/* Rótulo de campo para Data */}
                <label style={{fontSize:14,fontWeight:500,marginBottom:4,display:"block"}}>Data</label>
                ...
                {l.series.map((s,i)=>(
                  <div key={i} style={{marginBottom:12}}>
                    <label style={{fontSize:14,fontWeight:500,display:"block",marginBottom:4}}>Série {i+1}</label>
                ...
                {/* Rótulo para Observações */}
                <label style={{fontSize:14,fontWeight:500,marginBottom:4,display:"block"}}>Observações</label>
                <input value={linhaEditada.obs||l.obs||""}
                  onChange={e=>setLinhaEditada(p=>({...p,obs:e.target.value}))}
                  placeholder="Digite observações"
                  style={{width:"100%",marginBottom:12,padding:8,borderRadius:4,border:"1px solid #ccc"}}/>

                <button onClick={()=>salvarEdicao(idx)} style={{marginRight:10}}>
              ...
              <>
                ...
                <div style={{display:"flex",justifyContent:"space-between",gap:10,marginTop:10}}>
                  <button onClick={()=>{setEditandoIdx(idx); setLinhaEditada(iniciarEdicao(l));}}>
                    <FaPencilAlt className="inline-block mr-1" size={16}/>Editar
                  </button>
                  <button onClick={()=>excluirLinha(idx)} style={{backgroundColor:"#e11d48"}}>
