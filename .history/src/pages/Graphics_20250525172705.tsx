...
export default function Graphics() {
  ...
  useEffect(() => {
    ...
    Object.entries(bruto).forEach(([exe, ciclos]) => {
      Object.entries(ciclos as Record<string,RegistroTreino>).forEach(([cicloKey, reg]) => {
  ...
  return (
    <div style={{ width:"100vw", margin:0, padding:0, boxSizing:"border-box" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", padding:"12px" }}>
        ...
        <input
          style={{
            flex:1, marginLeft:8, padding:"8px", borderRadius:4,
      ...
      {filtrados.map(ex => {
        ...
        return (
          <div key={ex} style={{ background:"#1f1f1f", marginBottom:24 }}>
            <h2 style={{
              margin:0, padding:"12px", color:"#fff",
              textAlign:"center", fontSize:18
            }}>
              <ChartBar size={20} weight="duotone" className="inline-block mr-2" />
              Progresso — {ex}
            </h2>
            <div style={{ width:"100%", height:isMobile?260:360 }}>
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart
                  data={dados}
                  margin={{ top:16, right:16, left:60, bottom:isMobile?40:60 }}
                >
                  {/* Apenas linhas horizontais */}
                  <CartesianGrid stroke="#333" strokeDasharray="3 3" horizontal vertical={false} />

                  {/* Eixo esquerdo com mais divisões */}
                  <YAxis
                    yAxisId="media"
                    orientation="left"
                    width={50}
